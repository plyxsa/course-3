...
// Указатель на функцию из DLL
HANDLE(*sss)(char*, LPVOID);
HMODULE st;

// Пути к DLL и имя канала для консольного ввода-вывода
const char* dllname = "D:\\Уник\\1sem\\ПСП\\псп-лаб8-10\\lab8\\ServiceLibrary\\x64\\Debug\\ServiceLibrary.dll";
wstring pipename = L"RConsole";

// Переменные для отслеживания клиентов
volatile LONG CurrentClients;
volatile LONG TotalClients;
volatile LONG RejectedClients;
bool serverIsStop = false;

// Событие для приемника
...
void InitializeServerSettings(int argc, char* argv[]) {
    if (argc > 1) {
        try {
            port = stoi(argv[1]);
        }
...
// Загрузка DLL и получения указателя на функцию
bool LoadDllAndFunction() {
...
bool CreateServerThreads() {
    ...
    return hAcceptServer && hDispatchServer && hGarbageCleaner && hConsolePipe && hResponseServer;
}

// Ожидание завершения всех потоков и закрытие
void WaitForThreadsAndCleanup() {
    ...
    if (!CloseHandle(hAcceptServer)) cout << SetErrorMsgText("CloseHandle failed", GetLastError()) << endl;
    if (!CloseHandle(hDispatchServer)) cout << SetErrorMsgText("DispatchServer failed", GetLastError()) << endl;
    if (!CloseHandle(hGarbageCleaner)) cout << SetErrorMsgText("GarbageCleaner failed", GetLastError()) << endl;
    if (!CloseHandle(hConsolePipe)) cout << SetErrorMsgText("hConsolePipe failed", GetLastError()) << endl;
    if (!CloseHandle(hResponseServer)) cout << SetErrorMsgText("hResponseServer failed", GetLastError()) << endl;
}

int main(int argc, char* argv[]) {
    // Инициализация настроек
    InitializeServerSettings(argc, argv);

    // Загрузка DLL и функции
    if (!LoadDllAndFunction()) {
        return 1;
    }

    // Инициализация критической секции и сокетов
    InitializeCriticalSection(&ListContactCriticalSection);
    if (!InitializeSockets()) {
        return 1;
    }

    // Инициализация переменных для клиентов
    CurrentClients = 0;
    TotalClients = 0;
    RejectedClients = 0;
    ...
    if (!CreateServerThreads()) {
        cout << "Error creating server threads" << endl;
        return 1;
    }
