...
{
    const int ThreadCount = 15;       // Количество потоков
    ...
    static void Main(string[] args)
    {
        ...
        for (int i = 0; i < ThreadCount; ++i)
        {
            ...
            if (i % 3 == 0)
            {
                threads[i].Priority = ThreadPriority.Lowest; // Индексы 0, 3, 6, 9
            }
            else if (i % 3 == 2)
            {
                threads[i].Priority = ThreadPriority.Highest; // Индексы 2, 5, 8
            }
            // Иначе — приоритет по умолчанию (Normal)
